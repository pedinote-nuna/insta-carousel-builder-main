...
인스타 광고 정책 안전 규칙 (훅 슬라이드 reels-hook 의 헤드라인·purpose 에 필수):
- 의료 권위 주장 금지: "AAP/WHO 권장", "소아과 검증", "의사 추천" ✗ / 뱃지·신뢰 마크·인증 마크 시각 요소 금지 (출처는 카드뉴스 9번에만)
- 결과 보장 문구 금지: "N분 안에 해결", "즉시 효과", "완치", "바로 좋아져요", 수치+결과 조합("3일 만에"·"1번으로") ✗ → "제대로 하는 법", "순서 정리", "알아두면 좋은"
- 공포/긴급 유발 금지: "바로 병원", "응급", "위험!", "놓치면 큰일", 느낌표+경고 조합 ✗ → "이럴 때 확인하세요", "알아두면 안심", "체크리스트"
- 진단/처방 암시 금지: "이 증상=OO병", "집에서 치료", "약 없이 낫는" ✗ → "이런 경우엔", "소아과 가이드", "케어 방법"
- 헤드라인 끝에 느낌표(!) 단독 사용 금지 → 물음표(?) 또는 마침표(.)
- 승인 패턴: 정보제공형("…완벽 정리"·"…제대로 하는 법"), 공감형("이런 경험 있으신가요?"), 궁금증형("알고 계셨나요?"·"의외로 많이 틀리는")
...
# ── 광고 안전(Instagram Ad Policy) 훅 슬라이드 규칙 ────────────────────────
# 광고 거부 유발 표현을 안전한 표현으로 자동 치환. role=reels-hook 슬라이드에만 적용.
# (한국어 구문이라 영문 프롬프트 본문은 건드리지 않고 헤드라인·purpose 만 매칭됨)
_ADSAFE_TEXT_REPLACEMENTS = [
    # 의료 권위 주장 → 출처는 outro(9번)에만
    ("AAP 권장", "정리"),
    ("WHO 권장", "정리"),
    ("소아과 검증", "소아과 가이드"),
    ("소아과 추천", "알아두면 좋은"),
...
# purpose 필드 트리거 — 포함 시 (1)에서 못 잡은 변형까지 한 번 더 안전 보정
_ADSAFE_PURPOSE_TRIGGERS = ("긴급", "공포", "즉각 해결", "즉시 해결")

# 훅 슬라이드에서 제거할 뱃지/신뢰 마크 시각 요소 키워드 (영문/한글)
_ADSAFE_BADGE_KEYWORDS = (
    "badge", "trust mark", "trustmark", "trust-mark", "verification mark",
    "verified", "credential", "seal", "stamp of approval",
    "뱃지", "배지", "신뢰 마크", "인증 마크", "✓ 소아", "✓소아",
)


def _adsafe_hook_slide(extra: dict) -> None:
    """role=reels-hook 슬라이드에 인스타 광고 정책 안전 규칙을 적용(제자리 수정).

    1) 금지 표현(권위 주장·결과 보장·공포·진단 암시) → 안전 표현 치환
    2) purpose 의 긴급·공포·즉각 해결 → 안전 표현 치환
    3) 뱃지·신뢰 마크 시각 지시 제거
    4) 헤드라인 끝 느낌표(!) 단독 사용 → 마침표(.)
    """
    if str(extra.get("role", "")) != "reels-hook":
        return

    prompt = str(extra.get("prompt", ""))
    purpose = str(extra.get("purpose", ""))

    # (1)(2) 금지 표현 치환 — 한국어 구문이라 영문 프롬프트 본문은 건드리지 않음
    for bad, safe in _ADSAFE_TEXT_REPLACEMENTS:
        prompt = prompt.replace(bad, safe)
        purpose = purpose.replace(bad, safe)
    if any(t in purpose for t in _ADSAFE_PURPOSE_TRIGGERS):
        for bad, safe in _ADSAFE_TEXT_REPLACEMENTS:
            purpose = purpose.replace(bad, safe)

    # (3) 뱃지/신뢰 마크 시각 지시 제거 — 쉼표·마침표 구분 조각(fragment) 단위로 처리
    #     (나노바나나 프롬프트는 보통 쉼표로 나열되므로 쉼표까지 분할해야 해당 조각만 제거됨)
    #     단, 큰따옴표 헤드라인이 든 조각은 절대 통째 삭제하지 않고 뱃지 단어만 떼어낸다.
    def _has_quote(seg: str) -> bool:
        return bool(re.search(r"[\"“”]", seg))

    kept = []
    for seg in re.split(r"[,.;\n]", prompt):
        has_badge = any(k.lower() in seg.lower() for k in _ADSAFE_BADGE_KEYWORDS)
        if not has_badge:
            kept.append(seg)
            continue
        if not _has_quote(seg):
            continue  # 헤드라인 없는 순수 뱃지 조각 → 통째 제거
        # 헤드라인 포함 조각 → 뱃지 키워드만 제거하고 헤드라인은 보존
        for k in _ADSAFE_BADGE_KEYWORDS:
            seg = re.sub(re.escape(k), "", seg, flags=re.IGNORECASE)
        seg = re.sub(r"\b(with|featuring|including|rounded|small|and|a|an)\b\s*$",
                     "", seg.strip(), flags=re.IGNORECASE)
        kept.append(seg)
    prompt = ", ".join(s.strip() for s in kept if s.strip())

    # (4) 한글 뒤 느낌표 → 마침표 (헤드라인 단독 느낌표 제거)
    prompt = re.sub(r"(?<=[가-힣])\s*[!！]+", ".", prompt)
    purpose = re.sub(r"(?<=[가-힣])\s*[!！]+", ".", purpose)
...
def generate_extra_prompts(
    ...
    # AD-SAFE: templates/ad-safe-hook-rules.md 참조
    # 금지: 의료 권위 뱃지, 결과 보장 문구, 공포/긴급 CTA, 진단 암시
    # 허용: 정보 제공형 / 공감형 / 궁금증 유발형 헤드라인
...
반드시 3~4장만 생성하세요. 첫 장은 insert_after=0 훅, 나머지는 실사 설명 장면."""
    ...
    # + 훅 슬라이드(reels-hook)에 인스타 광고 정책 안전 규칙 적용
    for e in extras:
        if isinstance(e, dict):
            e["prompt"] = _append_korean_ethnicity(str(e.get("prompt", "")))
            _adsafe_hook_slide(e)
    return extras
...
def _apply_brand_fixes(text: str) -> str:
    ...
    for old, new in _TIME_NUMBER_FIXES:
        text = text.replace(old, new)
    return text
...
### 길이 규칙 (가장 중요 — 어기면 영상이 너무 길어짐)
- 각 줄은 공백 포함 25자 이내. 절대 35자 넘기지 말 것.
- 한 줄에 한 가지 핵심만. 슬라이드에 정보가 여러 개여도 가장 중요한 하나만 골라 짧게.
- 쉼표로 여러 정보를 나열하거나 한 문장에 욱여넣지 말 것.
- 이유·부연설명(~때문이에요, ~하는데)은 빼고 핵심 행동/사실만.
- 각 줄 구어체(~해요/~하세요).
...
대본 텍스트만 출력. 설명 없이."""
    ...
    user_prompt = (
        ...
        f"줄을 추가하거나 합치지 마.\n"
        f"각 줄은 25자 이내로 아주 짧게. 슬라이드 정보가 많아도 핵심 하나만 골라 한 문장으로. "
        f"절대 길게 쓰지 마."
    )
